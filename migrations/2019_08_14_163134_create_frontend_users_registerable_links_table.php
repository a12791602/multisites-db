...
class CreateFrontendUsersRegisterableLinksTable extends Migration {
	...
	public function up()
	{
		Schema::create('frontend_users_registerable_links', function(Blueprint $table)
		{
			...
			$table->boolean('is_tester')->nullable()->default(0)->comment('是非是测试用户');
			$table->integer('user_id')->unsigned()->index('idx_user')->comment('用户id （frontend_users表id）');
	...
	{
		Schema::drop('frontend_users_registerable_links');
	}
